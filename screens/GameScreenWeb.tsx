...
// Haptics not available on web - using vibration instead
...
import audioManager from '../utils/audioManagerWeb';
...
import FunGameOverModal from '../components/FunGameOverModalWeb';
...
export default function GameScreen({
    ...
    collisionHandler.current = new CollisionHandler({
      onScoreChange: (scoreChange) => {
        setScore((prev) => {
          ...
          const encouragement = funDifficultySystem.getEncouragement(newScore, combo, missedItems);
          if (encouragement) {
      ...
      onComboUpdate: (combo, multiplier) => {
        ...
        if (combo > 0 && combo % 5 === 0) {
          const encouragement = funDifficultySystem.getEncouragement(score, combo, missedItems);
          if (encouragement) {
            setEncouragementMessage(encouragement);
          }
        }
      ...
      onAchievement: (achievement) => {
        ...
        particleSystem.createParticles(ParticleType.ACHIEVEMENT_STARS, {
          x: width / 2,
          y: height / 3,
        });
      },
  ...
  const handleStateSpecialItem = (itemType: string) => {
    const stateUnlock = unlockedStates.find(
      (unlock) => unlock.visualElements.specialItem === itemType
    );
    ...
    if (stateUnlock) {
      ...
      console.log(
        `🏆 State Special: ${stateUnlock.stateName} - +${bonusScore} score, +${bonusCoins} coins`
      );
    }
  };
  ...
  const handleStateBonusItem = (itemType: string) => {
    ...
    setFallingItems((prev) =>
      prev.map((item) => (item.type === itemType ? { ...item, collected: true } : item))
    );
    ...
    // Show crate notification
    const showCrateNotification = (crate: {
      type: string;
      value: string | number;
      rarity: string;
    }) => {
      ...
      Alert.alert('🎁 Mystery Crate!', message, [{ text: 'Awesome!' }]);
        ...
        <View style={styles.container}>
          ...
          <View style={styles.gameArea}>
            ...
            <View style={styles.topControls}>
              ...
              {/* Sound Toggle */}
              <SoundToggle style={styles.soundToggle} size="medium" />
            </View>
            ...
            <View style={styles.scoreContainer}>
              ...
              <View style={styles.scoreHistory}>
                {lastScore > 0 && <Text style={styles.lastScoreText}>Last: {lastScore}</Text>}
                {highScore > 0 && <Text style={styles.highScoreText}>Best: {highScore}</Text>}
              </View>
              <View style={styles.statsRow}>
                <AnimatedScoreCounter score={coins} showCurrency={true} size="small" />
              ...
              <View style={styles.missedContainer}>
                <Text
                  style={[
                    styles.missedText,
                    missedItems > maxMissedItems * 0.8 && styles.missedTextWarning,
                  ]}
                >
                  ...
                  <View
                    style={[
                      styles.missedProgressFill,
                      {
                        ...
                        backgroundColor:
                          missedItems > maxMissedItems * 0.8
                            ? '#FF0000'
                            : missedItems > maxMissedItems * 0.5
                              ? '#FFA500'
                              : '#90EE90',
                      },
                    ]}
                ...
                {maxMissedItems > BASE_MAX_MISSED_ITEMS && (
                  <Text style={styles.upgradeIndicator}>
                    +{maxMissedItems - BASE_MAX_MISSED_ITEMS} 🆙
                  </Text>
                )}
            ...
            {/* Falling Items */}
            <FallingItems items={fallingItems} onItemCollect={onItemCollect} />
            ...
            <View style={styles.collectedItemsContainer}>
              ...
              <ScrollView
                ...
                style={styles.collectedItemsScroll}
              >
