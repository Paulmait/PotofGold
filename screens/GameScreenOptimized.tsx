...
const GameScreenOptimized: React.FC<GameScreenOptimizedProps> = memo(({
  ...
  const updateGame = useCallback((deltaTime: number) => {
    ...
    setFallingItems(prev => {
      ...
      updated.forEach(item => {
        if (!item.collected && CollisionDetection.checkItemCollision(item, {
          x: potPosition,
          y: height - 100,
          width: cartSize,
      ...
      return updated.filter(item => 
        !item.collected && item.y < CULL_DISTANCE
  ...
  const startCleanupTimer = useCallback(() => {
    cleanupTimer.current = setInterval(() => {
      ...
      setFallingItems(prev => 
        prev.filter(item => item.y < CULL_DISTANCE && !item.collected)
          .slice(-MAX_FALLING_ITEMS)
      );
      ...
      <View style={styles.container}>
        ...
        {!isGameActive && (
          <TouchableOpacity style={styles.startButton} onPress={startGame}>
            <Text style={[styles.startButtonText, { fontSize: sf(18) }]}>
              Start Game
            </Text>
          </TouchableOpacity>
        )}
        ...
        {showGameOver && (
          <View style={styles.gameOverMenu}>
            <Text style={[styles.gameOverTitle, { fontSize: sf(28) }]}>
              Game Over!
            </Text>
            ...
            >
              <Text style={[styles.gameOverButtonText, { fontSize: sf(16) }]}>
                Continue
              </Text>
            </TouchableOpacity>
          </View>
