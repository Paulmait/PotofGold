...
class BlockageManager {
  ...
  private findTargetLayer(x: number, width: number): number {
    for (let i = 0; i < this.blockageLayers.length; i++) {
      ...
      const hasCollision = layer.blockages.some(b => 
        this.checkOverlap(x, width, b.x, b.width)
      );
  ...
  checkCartPassage(cartX: number, cartWidth: number): {
    canPass: boolean;
  ...
  damageBlockage(blockageId: string, damage: number = 1): {
    destroyed: boolean;
