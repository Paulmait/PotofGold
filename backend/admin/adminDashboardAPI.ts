...
import { db, sanitizeInput, encryptData, decryptData, performanceMonitor } from '../firebaseConfig';
import { validateAdminSession, hasPermission, logAdminAction, AdminRole } from './adminAuth';
...
export const getUserDetails = functions.https.onCall(async (data, context) => {
  ...
  const gamesCount = await db.collection('games').where('userId', '==', userId).count().get();
...
export const banUser = functions.https.onCall(async (data, context) => {
  ...
  if (ban && duration) {
    updates['security.banUntil'] = admin.firestore.Timestamp.fromMillis(Date.now() + duration);
  }
...
export const getTransactions = functions.https.onCall(async (data, context) => {
  ...
  const { userId, type, startDate, endDate, limit = 50 } = sanitizeInput(data);
  ...
  const transactions = snapshot.docs.map((doc) => {
    ...
    return {
      ...
      details:
        session.role === AdminRole.SUPER_ADMIN && data.encrypted
...
export const getAnalytics = functions.https.onCall(async (data, context) => {
  ...
  // Get various metrics
  const [activeUsers, newUsers, revenue, gamesPlayed, avgSessionLength] = await Promise.all([
...
async function getActiveUsers(since: any): Promise<number> {
  const snapshot = await db.collection('users').where('lastActive', '>=', since).count().get();
...
async function getGamesPlayed(since: any): Promise<number> {
  const snapshot = await db.collection('games').where('startTime', '>=', since).count().get();
  return snapshot.data().count;
}
