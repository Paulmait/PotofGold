...
export const useGameUnlocks = () => {
  ...
  const canSpawnItem = useCallback((itemType: string): boolean => {
  ...
  const applyTrailEffect = useCallback((x: number, y: number) => {
  ...
  const celebrateUnlock = useCallback(async (type: 'skin' | 'trail' | 'state' | 'achievement', itemId: string) => {
  ...
  const checkAchievements = useCallback(async (gameStats: any) => {
  ...
  const progressStateUnlock = useCallback(async (stateId: string, points: number) => {
  ...
  const getPowerUpMultiplier = useCallback((powerUpId: string): number => {
  ...
  const canAffordUnlock = useCallback((cost: { coins?: number; gems?: number }, playerResources: { coins: number; gems: number }): boolean => {
