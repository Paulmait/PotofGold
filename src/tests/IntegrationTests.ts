...
class IntegrationTester {
  ...
  private async testNavigationRoutes(): Promise<void> {
    ...
    if (definedRoutes.length !== requiredRoutes.length) {
      throw new Error(
        `Missing navigation routes. Expected ${requiredRoutes.length}, got ${definedRoutes.length}`
      );
    }
  }
  ...
  private printResults(suite: IntegrationTestSuite): void {
    ...
    console.log(
      `${suite.overallPassed ? '✅' : '❌'} Overall: ${suite.overallPassed ? 'PASSED' : 'FAILED'}`
    );
    ...
    console.log('\n📈 Test Categories:');
    const categories = {
      'Navigation & Routing': suite.tests.filter(
        (t) => t.testName.includes('Navigation') || t.testName.includes('Route')
      ),
      'Data & Pricing': suite.tests.filter(
        (t) => t.testName.includes('Pricing') || t.testName.includes('Naming')
      ),
      'System Integration': suite.tests.filter(
        (t) => t.testName.includes('Integration') && !t.testName.includes('Complete')
      ),
      Security: suite.tests.filter(
        (t) =>
          t.testName.includes('Security') ||
          t.testName.includes('Protection') ||
          t.testName.includes('Validation')
      ),
      Performance: suite.tests.filter(
        (t) => t.testName.includes('Performance') || t.testName.includes('Memory')
      ),
      'End-to-End': suite.tests.filter(
        (t) =>
          t.testName.includes('Complete') ||
          t.testName.includes('Flow') ||
    ...
    Object.entries(categories).forEach(([category, tests]) => {
      if (tests.length > 0) {
        ...
        console.log(
          `  ${passed === tests.length ? '✅' : '❌'} ${category}: ${passed}/${tests.length}`
        );
      }
    });
