...
class LootBoxSystem {
  ...
  getLootBoxTypes(): LootBox[] {
    return [
      ...
      },
      {
        ...
        visualTheme: 'silver',
        odds: { common: 0, rare: 0.7, epic: 0.25, legendary: 0.045, mythic: 0.005 },
      ...
      },
      {
        ...
        visualTheme: 'golden',
        odds: { common: 0, rare: 0, epic: 0.8, legendary: 0.18, mythic: 0.02 },
  ...
  private calculateActualDropRates(): Record<string, string> {
    if (!this.playerHistory || this.playerHistory.totalOpened === 0) {
      return Object.entries(this.BASE_ODDS).reduce(
        (acc, [key, val]) => {
  ...
  private async savePlayerHistory() {
    ...
    try {
      await AsyncStorage.setItem(`loot_history_${Date.now()}`, JSON.stringify(this.playerHistory));
    } catch (error) {
