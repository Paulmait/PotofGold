  ...
  private getSmartPurchaseSuggestions(type: string, deficit: number): any[] {
    ...
    if (type === 'coins') {
      ...
      suggestions.push(
        ...
        {
          package: 'medium_coin_pack',
          amount: 5000,
          price: '$4.99',
          highlight: deficit > 1000 && deficit <= 5000,
        },
        {
          package: 'large_coin_pack',
          amount: 12000,
          price: '$9.99',
          highlight: deficit > 5000,
          bestValue: true,
        }
      );
    } else if (type === 'gems') {
      ...
      suggestions.push(
        ...
        {
          package: 'medium_gem_pack',
          amount: 500,
          price: '$9.99',
          highlight: deficit > 100 && deficit <= 500,
        },
        {
          package: 'large_gem_pack',
          amount: 1200,
          price: '$19.99',
          highlight: deficit > 500,
          bestValue: true,
        }
      );
    }
  ...
  async purchaseOffer(offerId: string, userId: string): Promise<PurchaseResult> {
    ...
    for (const content of offer.contents) {
      if (content.type in ['gems', 'coins', 'tickets', 'stars', 'energy', 'keys']) {
        await currencyManager.addCurrency(content.type as any, content.amount, 'purchase', userId);
      }
    }
