...
class RemoteConfigSync {
    ...
    try {
      ...
      if (response.success && response.config) {
        ...
        const retryDelay =
          response.retryAfter ||
          this.RETRY_DELAYS[
            Math.min(this.syncStatus.failureCount - 1, this.RETRY_DELAYS.length - 1)
          ];
  ...
  private async saveOfflineConfig(config: LiveOpsConfig, version?: ConfigVersion): Promise<void> {
    try {
      const offlineData = {
        config,
        version: version || {
      ...
      await AsyncStorage.setItem(this.OFFLINE_CACHE_KEY, JSON.stringify(offlineData));
