...
class CrashReportingSystem {
    ...
    try {
      await Promise.all([
        AsyncStorage.setItem(
          this.STORAGE_KEY,
          JSON.stringify(this.crashes.slice(-this.MAX_STORED_CRASHES))
        ),
        AsyncStorage.setItem(
          this.BREADCRUMBS_KEY,
          JSON.stringify(this.breadcrumbs.slice(-this.MAX_BREADCRUMBS))
        ),
  ...
  async handleError(
    error: Error,
    type: CrashType = CrashType.JAVASCRIPT_ERROR,
    context?: any
  ): Promise<void> {
  ...
  private async attemptRecovery(crashReport: CrashReport): Promise<boolean> {
    ...
    try {
      ...
      while (attempts < strategy.maxAttempts) {
        ...
        try {
          ...
          if (success) {
            this.addBreadcrumb(
              'recovery',
              `Successfully recovered from ${crashReport.type}`,
              'info'
            );
            return true;
          }
      ...
      this.addBreadcrumb(
        'recovery',
        `Recovery failed for ${crashReport.type}, using fallback`,
        'warning'
      );
