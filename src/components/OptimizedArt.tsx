...
import { View, StyleSheet, Dimensions, ViewStyle, ActivityIndicator, Platform } from 'react-native';
...
export const OptimizedArt: React.FC<OptimizedArtProps> = ({
  ...
  const imageSource = useMemo(() => {
    ...
    switch (variant) {
      case 'hero':
        ...
        const heroFile =
          deviceType === 'tablet' ? 'hero_tablet_1536x2048.png' : 'hero_phone_1080x1920.png';
      ...
      case 'thumbnail':
        ...
        const thumbFile =
          thumbRes === '@3x' ? 'thumb@3x.png' : thumbRes === '@2x' ? 'thumb@2x.png' : 'thumb.png';
        return `${basePath}/thumbnails/${thumbFile}`;

      case 'preview':
        // Adjust preview size based on device capabilities
        const previewSize =
          qualitySettings.maxTextureSize >= 2048
            ? 'tile_1024.png'
            : qualitySettings.maxTextureSize >= 1536
              ? 'tile_768.png'
              : 'tile_512.png';
      ...
      default:
        ...
        const suffix = resolution === '@3x' ? '@3x' : resolution === '@2x' ? '@2x' : '';
  ...
  const dimensions = useMemo(() => {
    ...
    const scaleFactor =
      deviceProfile.performanceTier === 'ultra'
        ? 1.2
        : deviceProfile.performanceTier === 'high'
          ? 1.0
          : deviceProfile.performanceTier === 'medium'
            ? 0.9
            : 0.75;
  ...
  return (
    <View style={[styles.container, dimensions, { backgroundColor }, containerStyle]}>
      <Image
        source={imageSource}
...
export async function preloadDropAssets(dropId: string): Promise<void> {
  const assets = [`cart_${dropId}`, `trail_${dropId}`, `badge_${dropId}`, `frame_${dropId}`];
