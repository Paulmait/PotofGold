...
class PerformanceMonitor {
  ...
  generateReport(): PerformanceReport {
    ...
    const avgImageLoadTime =
      this.metrics.imageLoadTimes.size > 0
  ...
  private detectCriticalIssues(): string[] {
    ...
    const slowScreens = Array.from(this.screenMetrics.values()).filter(
      (s) => s.averageRenderTime > 1000
    );
  ...
  async saveReport(): Promise<void> {
    ...
    try {
      await AsyncStorage.setItem(`@performance_report_${Date.now()}`, JSON.stringify(report));
    } catch (error) {
