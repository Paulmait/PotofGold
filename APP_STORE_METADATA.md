# Apple App Store Metadata - Pot of Gold

## App Information

**App Name:** Pot of Gold
**Bundle ID:** com.pofgold.potofgold
**Version:** 1.0.0
**Build Number:** 1.0.3
**Category:** Games - Arcade
**Secondary Category:** Games - Casual
**Age Rating:** 12+ (Infrequent/Mild Simulated Gambling)
**Developer:** Cien Rios LLC
**Support URL:** https://pofgold.com/support
**Privacy Policy URL:** https://pofgold.com/privacy
**Copyright:** 2024 Cien Rios LLC

---

## App Store Description

### Short Description (30 characters max)

Catch gold coins and win big!

### Promotional Text (170 characters max)

NEW: Winter Season Pass with exclusive skins! Compete in daily tournaments and climb the global leaderboard. Free to play with premium upgrades available.

### Full Description (4000 characters max)

**Pot of Gold** - The addictive coin-catching arcade game that will keep you coming back for more!

**CATCH COINS, UNLOCK TREASURES**
Swipe to move your pot and catch falling gold coins! Simple controls make it easy to pick up, but mastering the game takes skill. How high can you score?

**FEATURES:**

- Intuitive one-finger controls perfect for all ages
- Endless gameplay with increasing difficulty
- Multiple power-ups: Magnet, Speed Boost, Score Multiplier, Slow Motion
- 50+ unique pot skins to collect and customize
- State-themed collectibles from all 50 US states
- Daily challenges and streak rewards
- Global leaderboard to compete with players worldwide
- Weekly tournaments with exclusive prizes

**POWER-UPS:**

- Magnet: Attract coins automatically
- Double Points: 2x your score
- Slow Motion: More time to catch coins
- Gold Rush: Bonus coin shower

**UNLOCK & COLLECT:**

- Rare and legendary pot skins
- Seasonal event rewards
- Achievement badges
- Daily login bonuses

**COMPETE:**

- Challenge friends directly
- Climb the weekly leaderboard
- Participate in special events
- Share your high scores

**GOLD VAULT MEMBERSHIP (Optional):**

- Ad-free experience
- Exclusive member-only skins
- Daily bonus coins
- Priority access to new features

Download now and start your gold-catching adventure!

---

## Keywords (100 characters max)

coin,gold,arcade,casual,catch,coins,pot,treasure,game,fun,easy,simple,addictive,challenge,score

---

## What's New (4000 characters max)

Version 1.0.0 - Initial Release

- Addictive coin-catching gameplay
- 50+ unique pot skins to unlock
- Power-ups: Magnet, Double Points, Slow Motion, Gold Rush
- Daily challenges and streak system
- Global leaderboard
- Weekly tournaments
- Gold Vault premium membership option
- Privacy-first design with optional analytics
- Optimized for iPhone and iPad

---

## Screenshots Required

### iPhone 6.7" (1290x2796)

1. Gameplay - Catching coins with high score
2. Shop - Browsing pot skins
3. Power-ups - Magnet in action
4. Leaderboard - Ranking display
5. Achievement unlocked moment

### iPhone 6.5" (1284x2778)

Same as 6.7" scaled

### iPhone 5.5" (1242x2208)

Same as 6.7" scaled

### iPad Pro 12.9" (2048x2732)

Same as 6.7" with tablet layout

---

## App Privacy

### Data Collected

**Data Linked to You:**

- Identifiers (User ID, Device ID) - For app functionality
- Usage Data (Game progress, scores) - For app functionality
- Purchases (Transaction history) - For app functionality

**Data Not Linked to You:**

- Diagnostics (Crash data, Performance data) - For app improvement
- Usage Data (Analytics - if opted in) - For app improvement

### Data Not Collected

- Health & Fitness
- Financial Info
- Location
- Contacts
- User Content
- Search History
- Browsing History
- Sensitive Info

### Privacy Practices

- Data is encrypted in transit
- Data can be deleted upon request
- Users can opt out of data collection
- No data is shared with third parties for advertising (if not opted in)

---

## Review Information

**Demo Account:** Not required (guest play available)
**Notes for Reviewer:**

- This is an arcade-style coin-catching game
- Guest mode allows full gameplay without account creation
- Optional account creation for cloud saves and leaderboard
- In-app purchases for cosmetic items and optional subscription
- No real-money gambling - all purchases are for cosmetic items only
- Privacy consent is required before any data collection

**Contact Information:**

- Name: Support Team
- Phone: (Available in App Store Connect)
- Email: support@pofgold.com

---

## Age Rating Questionnaire Answers

- Cartoon or Fantasy Violence: None
- Realistic Violence: None
- Prolonged Graphic or Sadistic Violence: None
- Profanity or Crude Humor: None
- Mature/Suggestive Themes: None
- Horror/Fear Themes: None
- Medical/Treatment Information: None
- Alcohol, Tobacco, or Drug Use/References: None
- Simulated Gambling: Infrequent/Mild (gacha-style random rewards)
- Sexual Content or Nudity: None
- Contests: None
- Unrestricted Web Access: None

**Resulting Rating:** 12+ (for infrequent/mild simulated gambling)

---

## Export Compliance

- Uses HTTPS App Transport Security: Yes
- Uses Encryption: Yes (HTTPS only, no custom encryption)
- Is Encryption Exempt: Yes (ITSAppUsesNonExemptEncryption = false)
- Qualifies for Exception: Yes (Standard HTTPS/TLS only)

---

## Content Rights

- Does the app contain third-party content? No
- Does the app contain, display, or access third-party content? No
- All content is original or properly licensed

---

## Pricing

**Price Tier:** Free
**In-App Purchases:**

- Gold Vault Monthly ($4.99/month)
- Gold Vault Yearly ($29.99/year)
- Coin Pack Small ($0.99)
- Coin Pack Medium ($4.99)
- Coin Pack Large ($9.99)
- Coin Pack Mega ($19.99)
- Remove Ads ($2.99)

---

## Build & Submission Commands

```bash
# Build for iOS production
eas build --platform ios --profile production

# Submit to App Store
eas submit --platform ios --profile production

# Combined build and submit
eas build --platform ios --profile production --auto-submit
```

---

## Pre-Submission Checklist

- [x] App icon (1024x1024) uploaded
- [x] Screenshots for all required device sizes
- [x] Privacy policy URL accessible
- [x] Terms of service URL accessible
- [x] Support URL accessible
- [x] App description complete
- [x] Keywords optimized
- [x] What's New section filled
- [x] Age rating questionnaire completed
- [x] Export compliance answered
- [x] Content rights confirmed
- [x] In-app purchases configured
- [x] Build tested on device
- [x] Security audit passed
- [x] Performance tests passed
